...
from .exceptions import UnsupportedRecordFormatError, OcrError, MissingSectionError
from .vision_ocr import ocr_document
from .rules import (
    split_sections,
    is_legacy_format,
    extract_setuk,
...
def extract_from_bytes(pdf_bytes: bytes) -> list[dict]:
    # 옛 포맷이면 분석 전부 스킵하고 거절. 텍스트 PDF는 OCR 전에 내장 텍스트로 즉시 판별해
    # Vision 비용을 아끼고, 텍스트 레이어 없는 스캔본은 split_sections backstop이 받는다.
    if is_legacy_format(_pdf_text_layer(pdf_bytes)):
        raise UnsupportedRecordFormatError()

    text, pages_json = ocr_document(pdf_bytes)
