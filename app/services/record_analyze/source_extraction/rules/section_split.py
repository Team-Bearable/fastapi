...
입력: OCR 텍스트 (페이지 사이 form-feed `\\f` 구분).
...
from ..exceptions import UnsupportedRecordFormatError, MissingSectionError
...
def split_sections(text: str) -> dict[str, list[int]]:
    ...
    if s_창체 is None or s_교과 is None or s_행특 is None:
        raise MissingSectionError(f"필수 섹션 헤더 매칭 실패: 창체={s_창체}, 교과={s_교과}, 행특={s_행특}")
